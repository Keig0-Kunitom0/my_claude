...
| 引数 | 形式 | 既定 | 説明 |
|---|---|---|---|
| **対象 issue** | issue番号 / issue URL | 必須 | URL の場合は末尾の数値を issue 番号として扱います |
| `--base` | `--base <ref>` | **カレントの HEAD** | worktree の起点。[詳細](#派生元はカレント-head) |

worktree は detached で作ります。**作業ブランチは `/implement-issue` 側が作る**ので、ブランチ名を指定するオプションはありません（必要なら `/wt:run` を使ってください）。

#### `/wt:review`

| 引数 | 形式 | 既定 | 説明 |
|---|---|---|---|
| **対象 PR** | PR番号 / PR URL / ブランチ名 | 必須 | ブランチ名を渡した場合は PR 検出を省いてそのまま使います |

**起点は `origin/<PR の head ブランチ>` 固定**で、`--base` はありません。レビュー対象は PR で一意に決まるためです。worktree は detached で作ります（読むだけなのでブランチが不要）。

#### `/wt:run`

| 引数 | 形式 | 既定 | 説明 |
|---|---|---|---|
| **プロンプト** | 文字列 | 必須 | 新セッションにそのまま投入します。加工しません |
| `--base` | `--base <ref>` | **カレントの HEAD** | worktree の起点 |
| `--branch` | `--branch <名前>` | **detached**（ブランチを作らない） | コミットを伴う作業では指定してください |
| `--name` | `--name <名前>` | プロンプト先頭から自動生成 | ワークスペース名。小文字英字始まり・小文字英数と `-` `_` のみ・32文字以内 |

#### `/wt:clear`

| 引数 | 形式 | 既定 | 説明 |
|---|---|---|---|
| **対象** | 番号 / label | **省略時は一覧から選択** | 番号は `issue-<番号>` と `review-<番号>-*` に一致するものを探します |
| `--workspace` | `--workspace <id>` | — | ワークスペース ID で直接指定します。名前が重複した場合の曖昧さ回避に使います |
| `--prune` | — | **ブランチを残す** | 安全と判定できた作業ブランチも削除します。[詳細](#残るのは作業ブランチだけ) |
| `--force` | — | dirty なら拒否 | 未コミットの変更があっても撤収します。[詳細](#dirty-な-worktree) |

`--prune` は「**安全なら消す**」であって「強制的に消す」ではありません。未 push のコミットがあるブランチは、`--prune` を付けても削除されません。
