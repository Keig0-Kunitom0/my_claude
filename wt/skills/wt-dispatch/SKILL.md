...
## 手順3: CLAUDE.md をコピーし、worktree 向けの注意を追記する
...
## このチェックアウトは git worktree です（wt-dispatch による自動追記）

**上に書かれている起動・テスト・ビルドのコマンドは、本体のチェックアウトを前提に書かれている。**
このディレクトリでそのまま実行すると、本体の開発環境を破壊する場合がある。

worktree からの実行手順が明示されていないコマンドは実行せず、人間に確認すること。
EOF
fi
```
...
**注意を追記する理由**: コピー元の `CLAUDE.md` には、ほぼ例外なく本体チェックアウト前提の
起動・テストコマンドが書かれている（`docker compose up`、`npm run dev` 等）。**worktree 対応を
明記しているリポジトリはまず無い。** 追記しないと、エージェントは「CLAUDE.md に書いてある
コマンド」を安全なものとして実行する。

**危険なコマンドと同じファイルに載せること**に意味がある。別ファイルに置くと、
CLAUDE.md だけ読んで実行に移られた場合に届かない。
...
```bash
WORKTREE_NOTICE='このディレクトリは git worktree であり、本体のチェックアウトとは別のディレクトリです。
以下の性質があります。
- gitignore されたファイル（.env、ローカル設定、ビルド成果物など）は存在しません
- ディレクトリ名から識別子を導くツール（docker compose のプロジェクト名など）は
  本体のチェックアウトと衝突し、本体の開発環境を破壊することがあります

ローカル環境の起動・テスト・ビルドは、リポジトリのドキュメントに
「git worktree から実行する場合の手順」が明示されている場合にのみ行うこと。
通常の起動/テストコマンドが書かれていても、それは本体チェックアウト前提のものであり、
worktree で安全とは限りません。worktree 向けの記載が無い場合は実行せず、人間に確認すること。'
...
**`WORKTREE_NOTICE` は呼び出し元に書かせず、ここで一元的に付ける。** 3コマンドに同じ文面を
配るとコピペ増殖する。加えて `CLAUDE.md` が無いリポジトリでは手順3の追記が効かないため、
**プロンプト経由が唯一確実に届く経路**になる。

**「リポジトリのドキュメントに従え」とだけ書いてはならない。** 既存の `CLAUDE.md` や README は
例外なく「チェックアウトは1つ」を前提に書かれているため、単に従わせると本体前提のコマンドを
踏ませることになる。**worktree 向けの明示的な記載があることを、実行の必要条件にする。**

**禁止ではなく「明示があればオプトインで許可」にする理由**: テスト実行を一律に禁じると、
テストを通してからコミットする前提の投入先コマンド（`/implement-issue` 等）が未検証の
コミットを積む。安全側のデフォルトは「未知なら止まって聞く」であり、「やるな」ではない。
