...
| **補足ドキュメント** | パス（複数可） | — | issue の内容を補う仕様書。issue 本文にパスが書かれていればそれも読みます |
| `--base` | `--base <ref>` | **カレントの HEAD** | 作業ブランチの派生元。[詳細](#派生元は今いる場所) |
| `--branch` | `--branch <名前>` | 既存の命名規約から推測 | 作業ブランチ名を明示指定します。[詳細](#ブランチ名は既存の規約から推測します) |
| `--context` | `--context <パス>`（複数可） | — | **前提知識**として読ませる Markdown。ファイルでもディレクトリでも可。[詳細](#ドメイン知識を---context-で渡す) |

`--base` と `--context` は名前が似ていますが役割は無関係です。**`--base` は git の起点**を決めるもの、**`--context` は読ませる知識**を渡すものです。
